...
from utils import LIT_101_M, RWT_INIT_LEVEL, FWT_INIT_LEVEL, LIT_301_M
...
class RawWaterTank(Tank):
    ...
    def main_loop(self):
        ...
        while(count <= PP_SAMPLES):
            ...
            # lit301_water_volume += outflow # what ever comes out of lit301 goes into lit101

            # level cannot be negative
            if new_level <= 0.0:
                new_level = 0.0

            # update internal and state water level and lit301_level
            print "DEBUG new_level: %.5f \t delta: %.5f" % (
                new_level, new_level - self.level)
            self.level = self.set(LIT101, new_level)
            # new_lit301_level = lit301_water_volume / self.section
...
class FinalWaterTank(Tank):

    def pre_loop(self):

        # SPHINX_SWAT_TUTORIAL STATE INIT(
        self.level = self.set(LIT301, 0.400)
        # SPHINX_SWAT_TUTORIAL STATE INIT)
    ...
    def main_loop(self):
        ...
        while(count <= PP_SAMPLES):
            ...
            if float(fit201) > 0: # or float(lit301) < LIT_301_M['LL']:
                inflow = PUMP_FLOWRATE_OUT * PP_PERIOD_HOURS # PUMP_FLOWRATE_OUT replictes outflow from raw water tank
                print "DEBUG FinalWaterTank inflow: ", inflow
                water_volume += inflow

            # outflows volumes (constant rate at 1.5m^1/h)
            # TODO: implement constant outflow rate
            # if int(lit301) > LIT_301_M['HH']:
            #     # self.set(FIT201, PUMP_FLOWRATE_OUT)
            #     outflow = PUMP_FLOWRATE_OUT * PP_PERIOD_HOURS
            #     print "DEBUG FinalWaterTank outflow: ", outflow
            #     water_volume -= outflow
            ...
            if new_level >= LIT_301_M['HH']:
                print 'DEBUG FinalWaterTank above HH count: ', count
                break

            # 367 sec starting from 0.500 m
            elif new_level <= LIT_101_M['LL']:
                print 'DEBUG FinalWaterTank below LL count: ', count
                break
...
if __name__ == '__main__':
    ...
    print "\n\n\nSTARTING FINAL WATER TANK...\n\n\n"
    fwt = FinalWaterTank(
        name='fwt',
        state=STATE,
        protocol=None,
        section=TANK_SECTION,
        level=FWT_INIT_LEVEL
    )
